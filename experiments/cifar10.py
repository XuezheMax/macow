...
parser.add_argument('--warmup_steps', type=int, default=50, metavar='N', help='number of steps to warm up (default: 50)')
...
final_loader = DataLoader(test_data, batch_size=16, shuffle=False, num_workers=args.workers, pin_memory=True)
final_k = 512
fgen.load_state_dict(torch.load(model_name))
with torch.no_grad():
    print('Final test:')
    eval(final_loader, final_k)
    print('-' * 100)
    reconstruct('final')
    sample('final')
