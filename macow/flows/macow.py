...
class MaCowUnit(Flow):
    ...
    def init(self, data, h=None, init_scale=1.0) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        out, logdet = self.conv1x1.init(out, h=h)
        logdet_accum = logdet_accum + logdet
...
class MaCowStep(Flow):
    ...
    """
    def __init__(self, in_channels, kernel_size, scale=True, inverse=False, dropout=0.0):
        ...
        self.coupling = NICE(in_channels, scale=scale, inverse=inverse, dropout=dropout)
...
class MaCow(Flow):
    ...
    def __init__(self, levels, num_steps, in_channels, kernel_size, scale=True, inverse=False, dropout=0.0):
        ...
        for level in range(levels):
            if level == 0:
                macow_block = MaCowBottomBlock(num_steps[level], in_channels, kernel_size, scale=scale, inverse=inverse, dropout=dropout)
